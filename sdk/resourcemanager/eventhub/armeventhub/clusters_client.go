...
// TriggerUpgradePost - Trigger pending cluster upgrades if any. Bypasses any upgrade preferences set by customer.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2024-01-01
//   - resourceGroupName - Name of the resource group within the azure subscription.
//   - clusterName - The name of the Event Hubs Cluster.
//   - options - ClustersClientTriggerUpgradePostOptions contains the optional parameters for the ClustersClient.TriggerUpgradePost
//     method.
func (client *ClustersClient) TriggerUpgradePost(ctx context.Context, resourceGroupName string, clusterName string, options *ClustersClientTriggerUpgradePostOptions) (ClustersClientTriggerUpgradePostResponse, error) {
	var err error
	const operationName = "ClustersClient.TriggerUpgradePost"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.triggerUpgradePostCreateRequest(ctx, resourceGroupName, clusterName, options)
	if err != nil {
		return ClustersClientTriggerUpgradePostResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return ClustersClientTriggerUpgradePostResponse{}, err
	...
	return ClustersClientTriggerUpgradePostResponse{}, nil
}

// triggerUpgradePostCreateRequest creates the TriggerUpgradePost request.
func (client *ClustersClient) triggerUpgradePostCreateRequest(ctx context.Context, resourceGroupName string, clusterName string, options *ClustersClientTriggerUpgradePostOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.EventHub/clusters/{clusterName}/triggerUpgrade"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
